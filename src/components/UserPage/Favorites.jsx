...
import { getStoredToken, getFavorites as apiGetFavorites, removeFavorite as apiRemoveFavorite } from '../../services/useTilapiaApi';
...
function writeStorage(list) {
  try {
    ...
    console.warn('Favorites storage write failed', err);
  }
}
  ...
  return (
    <Container>
      ...
      {list.length === 0 ? (
        <EmptyState>
          <span>📽️</span>
          <p>Nenhum filme adicionado ainda.<br/>Adicione filmes para a sua lista de favoritos!</p>
        </EmptyState>
      ) : (
        <MovieList>
          {list.map((movie) => (
